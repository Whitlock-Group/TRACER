...
class IndexTracker_pi(object):
    ...
    def __init__(self, ax, X, pixdim, p, S, unique_slice, probe_x, probe_y, probe_colors, probe_selecter, m, b):
        self.ax = ax
        self.plane = p.lower()
...
class IndexTracker_pi_col(object):
    ...
    def __init__(self, ax, X, pixdim, p, S, unique_slice, probe_x, probe_y, m, b):
        self.ax = ax
        self.plane = p.lower()
        self.unique_slice = unique_slice
        self.probe_x = probe_x
        ...
        if self.plane == 'c':
            ...
            self.ind = S
            self.L = self.X.transpose((2,1,0,3))
            self.im = ax.imshow(self.L[:, self.ind, :], origin="lower", extent=[0, 512*pixdim, 0, 512*pixdim], cmap='gray')
        elif self.plane == 's':
            self.slices, rows, cols, color= X.shape
            self.ind = S
            self.L = self.X.transpose((0,2,1,3))                  
            self.im = ax.imshow(self.L[self.ind, :, :], origin="lower", extent=[0 ,1024*pixdim, 0, 512*pixdim], cmap='gray')            
        elif self.plane == 'h':  
            rows, cols, self.slices, color = X.shape
            self.ind = S
            self.L = self.X.transpose((1,0,2,3)) 
            self.im = ax.imshow(self.L[:, :, self.ind], origin="lower", extent=[0, 512*pixdim, 0, 1024*pixdim], cmap='gray') 
        # plot the probe
    ...
    def onscroll(self, event):
        if event.button == 'up':
            ...
            if self.ind not in self.unique_slice:
                # plot the probe
                self.line = plt.plot(np.array(sorted(self.probe_x)), self.m*np.array(sorted(self.probe_x)) + self.b,color=lighten_color('black', 0.4), linestyle='dashed', linewidth=0.8)
            else:
                # plot the probe
                self.line = plt.plot(np.array(sorted(self.probe_x)), self.m*np.array(sorted(self.probe_x)) + self.b,color='black', linestyle='dashed', linewidth=0.8);    
        else:
